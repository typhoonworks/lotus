defmodule Lotus.SourceTest do
  ...
  describe "format_error/1" do
    test "formats Postgrex syntax errors" do
      ...
      assert Source.format_error(error) ==
    ...
    test "formats Postgrex errors with message field" do
      ...
      assert Source.format_error(error) == "SQL error: connection timeout"
    end

    test "formats Postgrex errors with exception message" do
      error = %Postgrex.Error{}
      assert is_binary(Source.format_error(error))
    end

    test "formats Exqlite errors" do
      error = %Exqlite.Error{message: "no such table: users"}
      assert Source.format_error(error) == "SQLite Error: no such table: users"
    end

    test "formats DBConnection.EncodeError" do
      error = %DBConnection.EncodeError{message: "expected int"}
      assert Source.format_error(error) == "expected int"
    end

    test "formats ArgumentError" do
      error = %ArgumentError{message: "invalid argument"}
      assert Source.format_error(error) == "invalid argument"
    end

    test "returns binary messages as-is" do
      assert Source.format_error("Already formatted") == "Already formatted"
    end

    test "formats other terms with inspect" do
      assert Source.format_error(:some_atom) == "Database Error: :some_atom"
      assert Source.format_error(123) == "Database Error: 123"
      assert Source.format_error({:error, "test"}) == "Database Error: {:error, \"test\"}"
    end

    test "formats generic exceptions" do
      error = %RuntimeError{message: "Something went wrong"}
      assert Source.format_error(error) == "Something went wrong"
    end
  end
